(function () {
  ...
  function game(state, activeDialogue) {
    ...
    app().innerHTML = `<section class="screen has-scene game adventure-game"><header class="hud"><div><div class="brand">El misterio de Salamanca</div><small class="place-name">${loc.name}</small></div><div class="objective"><span>Current objective</span><strong>${questText(state)}</strong></div><div class="stats"><span class="stat">XP ${state.player.xp}</span><span class="stat">◈ ${state.player.coins}</span></div><div class="hud-actions"><button class="btn small ghost" data-action="mission-help">Help</button><button class="btn small" data-action="open-save">Save</button></div></header><div class="adventure-stage stage-${state.location}" data-walk-stage><div class="sky-shade"></div>${stageCharacters(state)}${stageExits(state)}<div class="player-wrap" style="--player-x:${state.playerPosition || 18}%"><img class="player-character" src="${ASSET_BASE}player-investigator.png" alt="Your investigator"></div><div class="walk-prompt">Tap the ground to walk · Tap a person to interact</div></div><footer class="adventure-bar"><button class="verb active">Walk</button><button class="verb">Look</button><button class="verb">Talk</button><button class="verb">Use</button><div class="quick-items"><strong>Inventory</strong><span>${state.inventory.length ? state.inventory.map(i => escape(i.name)).join(" · ") : "Empty"}</span></div><button class="btn small ghost" data-action="open-glossary">Word help</button></footer>${activeDialogue && D.dialogue[activeDialogue] ? dialogue(D.dialogue[activeDialogue]) : ""}</section>`;
  }
  ...
  SpanishRPG.UI = { start, character, intro, game, sideContent, modal, closeModal, toast, shop, suspects, glossary, missionHelp, saveMenu, ending };
})();
