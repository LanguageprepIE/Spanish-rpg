(function () {
  ...
  function movePlayerTo(percent, afterMove) {
    const next = Math.max(7, Math.min(93, Number(percent) || 18));
    const player = document.querySelector(".player-wrap");
    const previous = state.playerPosition || 18;
    state.playerPosition = next;
    if (!player) { if (afterMove) afterMove(); return; }
    player.classList.toggle("facing-left", next < previous);
    player.classList.add("moving");
    player.style.setProperty("--player-x", next + "%");
  ...
  document.addEventListener("click", function (event) {
    ...
    else if (type === "mission-help") RPG.UI.missionHelp(state);
  ...
  document.addEventListener("click", function (event) {
    const stage = event.target.closest("[data-walk-stage]");
    if (!stage || event.target.closest("button")) return;
    const rect = stage.getBoundingClientRect();
    movePlayerTo(((event.clientX - rect.left) / rect.width) * 100);
  });
